		   ...
		   $(function(){
			   $("#idCheck").click(function(){
				  ...
				  $.ajax({
					 ...
					 type:"post",
					 data:{"id":$("#id").val()},
					 dataType:"json",//ajax에서 받는파일형태
					 contentType:"json",//ajax 보내는 파일형태
					 success:function(data){
						 if(data.result=="fail") {
							 alert("아이디가 이미 존재합니다.")
							 $("#id").val("");
							 $("#id").focus();
							 $("#checkTxt").text("아이디 사용불가능");
							 $("#checkTxt").css({"color":"red","font-weigth":"900"});
							 return false;
						 }else {
							 alert("아이디 사용이 가능합니다.")
							 $("#checkTxt").text("아이디 사용가능");
							 $("#checkTxt").css({"color":"blue","font-weigth":"900"});
						 }
					 },
					 error:function(){
						 alert("실패")
